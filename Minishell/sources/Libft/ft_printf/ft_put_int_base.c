...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/01 16:44:15 by daromoli          #+#    #+#             */
/*   Updated: 2025/02/16 19:28:05 by daromoli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		calc_print_figure(int fd, long nbr, int length, char *base);

/*
PARAMETERS
fd:		The file descriptor to write to.
nbr:	The decimal integer to print.
base:	The base to convert and print nbr into.

DESCRIPTION
This function prints the decimal number nbr in a base system in the terminal,
while it counts the number of characters printed (count).
...
int	ft_put_int_base(int fd, int nbr, char *base)
{
	...
	if (length != 0)
	{
		tmp_count = calc_print_figure(fd, number, length, base);
		if (tmp_count == ERROR)
			return (ERROR);
		count += tmp_count;
	}
...
PARAMETERS
fd:		The file descriptor to write to.
number:	The decimal integer to convert and print.
...
EXPLANATION
If the number is negative prints '-' and converts it to positive. Then it
divides the number by the length of the base ("length") and recalls itself
until the result of the division is 0. Then it prints the letter in base
correspondent to the remainder of the division for each recursion.
Every write increments the variable count by the amount of characters printed.
count is then returned to be used by the precedent call of this recursive
function, or by the ft_putnbr_base_count function.
If an error occurs at any moment, so tmp_count = ERROR = -1, then ERROR = -1
is returned and the execution stops.
*/
static int	calc_print_figure(int fd, long number, int length, char *base)
{
	int		count;
	int		tmp_count;
	...
	if (number / length != 0)
	{
		tmp_count = calc_print_figure(fd, number / length, length, base);
		if (tmp_count == ERROR)
			return (ERROR);
		count += tmp_count;
	}
	tmp_count = write(fd, &base[number % length], 1);
	if (tmp_count == ERROR)
		return (ERROR);
	count += tmp_count;
	return (count);
