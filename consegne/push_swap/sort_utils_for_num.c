...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/11 16:33:33 by lhima             #+#    #+#             */
/*   Updated: 2025/02/11 16:33:37 by lhima            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	best_cost(t_lista **a, t_lista **b, t_cost *costs)
{
	...
	while (tmp_b != NULL)
	{
		...
		else if (costs->bestcost > ft_abs(find_cost_of_a_num(*a, \
				close_max(tmp_b->num, *a)), \
				find_cost_of_a_num(*b, tmp_b->num)))
		{
			costs->bestcost = ft_abs(find_cost_of_a_num(*a, \
			close_max(tmp_b->num, *a)), \
			find_cost_of_a_num(*b, tmp_b->num));
			costs->cost_b = find_cost_of_a_num(*b, tmp_b->num);
